...
def lsa_query(main_question, model='mistral', chatbot=ollama.chat):
    conversation = list()
    conversation.append({'role': 'system', 'content': SYSTEM.replace('<<QUERY>>', main_question)})
...
if __name__ == '__main__':
    ...
    for message in enumerate(lsa_query(main_question, model='mistral', chatbot=chatbot)):
        if message[0] % 2 == 0:
